...
Avatar = {
  ...
  getUrl: function (user) {
    ...
    if (user) {
	if (Avatar.options.customAvatarProperty) {
            customUrl = getObjectWithPath(user, Avatar.options.customAvatarProperty);
            if (customUrl) {
		Meteor.subscribe('avatars',[customUrl]);
		var res=Avatars.findOne({_id:customUrl});
		if (res)
		    return res.url();
	    };
	};
