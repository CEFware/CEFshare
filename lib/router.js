...
Router.map (function () {	    
    ...
    this.route('user/:username/invoices',{
	name: 'userInvoices',
	path : '/user/:username/invoices',
	action: function () {
	    if (Meteor.user() && ((Meteor.user().username===this.params.username) || Roles.userIsInRole(Meteor.userId(),'admin'))) {
		this.subscribe('getUserInvoices').wait();
		if (this.ready()) {
		    this.render('userInvoicesTemplate');
		} else {
		    this.render('loading');
		};
	    } else {
    ...
    this.route('user/:username/invoices/:id',{
	name: 'userInvoice',
	path : '/user/:username/invoices/:id',
	action: function () {
	    if (Meteor.user() && ((Meteor.user().username===this.params.username) || Roles.userIsInRole(Meteor.userId(),'admin'))) {
		this.subscribe('getInvoiceById',this.params.id).wait();
		if (this.ready()) {
		    if (Invoices.find({invoiceNum:this.params.id}).count()>0) {
			this.render('userInvoiceTemplate');
		    } else {
			this.render('notFound');
		    };
		} else {
